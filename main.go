...
func printUsage() {
	fmt.Println("🚀 Task Tracker CLI — Управляй задачами эффективно (v1.0)")
	fmt.Println("\nИспользование:")
	fmt.Println("  task-cli <команда> [аргументы]")
	fmt.Println("\nДоступные команды:")

	// Основные команды (CRUD)
	fmt.Printf("  %-25s %s\n", "add <описание>", "Добавить новую задачу (статус: TODO)")
	fmt.Printf("  %-25s %s\n", "list", "Показать все задачи в виде таблицы")
	fmt.Printf("  %-25s %s\n", "list <статус>", "Фильтр задач: todo, in-progress, done")
	fmt.Printf("  %-25s %s\n", "update <ID> <описание>", "Изменить текст существующей задачи")
	fmt.Printf("  %-25s %s\n", "delete <ID>", "Удалить задачу из списка")

	// Управление статусами
	fmt.Printf("  %-25s %s\n", "mark-in-progress <ID>", "Перевести задачу в статус 'В ПРОЦЕССЕ'")
	fmt.Printf("  %-25s %s\n", "mark-done <ID>", "Перевести задачу в статус 'ВЫПОЛНЕНО'")

	// Бонусные фичи (День 10)
	fmt.Printf("  %-25s %s\n", "search <слово>", "Найти задачи по ключевому слову в описании")
	fmt.Printf("  %-25s %s\n", "export csv", "Выгрузить все задачи в формате CSV (для Excel)")

	fmt.Println("\nПримеры:")
	fmt.Println("  ./task-cli add \"Изучить интерфейсы в Go\"")
	fmt.Println("  ./task-cli list todo")
	fmt.Println("  ./task-cli search \"интерфейс\"")
